...
echo -ne "\xEF\xBB\xBF" > components.csv
echo "поз;тип компонента;наименование компонента;маркировка;количество;дата покупки;что делает;для чего использовать;примечание" >> components.csv
...
find components -name '*.txt' -print0 | sort -zV | while IFS= read -r -d $'\0' file; do
    ...
    echo "$csv_line" >> components.csv
    echo "Обработан: $(basename "$file") (поз: ${data[поз]:-НЕТ})"
done

# Сортировка по первому столбцу (позиция)
(
    head -n 1 components.csv
    tail -n +2 components.csv | sort -n -t ';' -k1,1
) > components_sorted.csv

mv components_sorted.csv components.csv
echo "Готово! Все данные объединены в components.csv"